...
public class ClientStart {
  ...
  private ClientStart(InetAddress serverAddress, int serverPort) throws Exception {
    ...
    if (args.length < 1) {
      System.out
          .println("Error: enter a valid host server address to continue..\nUsage: "
              ...
              + "\nExample usage: java -cp pathtoshadowjar com.newrelic.codingchallenge.clientapi.ClientStart localhost [true | false]");
      System.exit(0);
    }

    ClientStart client =
        new ClientStart(InetAddress.getByName(args[0]), defaultPort);
  ...
  private void createThreads() {
    ...
    if (automateClient) {
      AutoDataGenerate generateRunnable = new AutoDataGenerate(this);
